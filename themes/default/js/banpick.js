...
function handleData(data) {
    // Initialize MotionHero when we receive motionHeroEnabled flag
    ...
    switch (data.type) {
        ...
        case 'resetBanPick':
            resetBanPickSlots();
            MotionHero.clearMemory(); // Dọn RAM ngay lập tức khi bắt đầu ván mới
            break;
...
function resetBanPickSlots() {
    ...
    for (let i = 1; i <= 5; i++) {
        ['A', 'B'].forEach(team => {
            ...
            if (slot) {
                // Clear hero image AND any video elements
                const heroImageDiv = slot.querySelector('.heroImage');
                if (heroImageDiv) {
                    // Remove video elements and glass-shields
                    const video = heroImageDiv.querySelector('video');
                    if (video) {
                        video.pause();
                        video.removeAttribute('src');
                        video.load(); // Force release media resources
                        video.remove();
                    }
                    const shield = heroImageDiv.querySelector('.glass-shield');
