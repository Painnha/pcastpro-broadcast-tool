...
powershell -Command "$root = '%~dp0..'; $temp = Join-Path $root 'netlify-temp'; $dist = Join-Path $root 'netlify-update-dist'; if (Test-Path $temp) { try { Remove-Item -Path $temp -Recurse -Force -ErrorAction SilentlyContinue } catch {} }; New-Item -ItemType Directory -Path $temp | Out-Null; if (-not (Test-Path $dist)) { New-Item -ItemType Directory -Path $dist | Out-Null }; $null = (& robocopy (Join-Path $root 'frontend') (Join-Path $temp 'frontend') /E /XD heroMotion /R:1 /W:1 /NFL /NDL /NJH /NJS /NS /NC /NP); Write-Host '[OK] Da copy frontend (loai tru heroMotion).'; Copy-Item -Path (Join-Path $root 'shared') -Destination (Join-Path $temp 'shared') -Recurse -Force; Copy-Item -Path (Join-Path $root 'themes') -Destination (Join-Path $temp 'themes') -Recurse -Force; $tempScripts = Join-Path $temp 'scripts'; New-Item -ItemType Directory -Path $tempScripts | Out-Null; Copy-Item -Path (Join-Path $root 'scripts/quick-start.bat') -Destination (Join-Path $tempScripts 'quick-start.bat') -Force; Copy-Item -Path (Join-Path $root 'scripts/start-backend.bat') -Destination (Join-Path $tempScripts 'start-backend.bat') -Force; Copy-Item -Path (Join-Path $root 'PCastPro.exe') -Destination (Join-Path $temp 'PCastPro.exe') -Force; Add-Type -AssemblyName System.IO.Compression.FileSystem; $zipPath = Join-Path $dist 'pcastpro-latest.zip'; if (Test-Path $zipPath) { Remove-Item -Path $zipPath -Force }; [System.IO.Compression.ZipFile]::CreateFromDirectory($temp, $zipPath); Start-Sleep -Seconds 2; try { Remove-Item -Path $temp -Recurse -Force -ErrorAction SilentlyContinue } catch {}; $versionPath = Join-Path $dist '%TARGET_JSON_FILE%'; $history = @(); if (Test-Path $versionPath) { $oldData = Get-Content -Raw -Path $versionPath | ConvertFrom-Json; if ($oldData) { if ($oldData.version -ne '%NEW_VERSION%') { $historyEntry = @{ version = $oldData.version; date = (Get-Date -Format 'yyyy-MM-dd'); releaseNotes = $oldData.releaseNotes }; if ($oldData.history) { $history = $oldData.history }; $history += $historyEntry } else { if ($oldData.history) { $history = $oldData.history } } } }; $notesArray = '%RELEASE_NOTES%'.Split(',') | ForEach-Object { $_.Trim() }; $versionObj = [PSCustomObject]@{ version = '%NEW_VERSION%'; downloadUrl = '%TARGET_URL%'; releaseNotes = $notesArray; history = $history }; $versionObj | ConvertTo-Json -Depth 5 | Out-File -FilePath $versionPath -Encoding utf8"
